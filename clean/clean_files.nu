...
def path-has-protected-part [path: string] {
  ...
  ($path | str contains "/backup/archlinux/data/") or ($path | str ends-with "/backup/archlinux/data")
}
...
def main [
  ...
  for file in $files {
    let failed = (try {
      ...
      status "ERROR" $"Failed to delete file: ($file)."
      true
    })
  ...
  for dir in $junk_dirs {
    if ($dir | path exists) {
      let failed = (try {
        ...
        status "ERROR" $"Failed to remove directory: ($dir)."
        true
      })
      if $failed { $failures += 1 }
    }
  ...
  for dir in $empty_dirs {
    if ($dir | path exists) {
      let failed = (try {
        ...
        status "ERROR" $"Failed to remove empty directory: ($dir)."
        true
      })
      if $failed { $failures += 1 }
    }
  ...
  if $failures > 0 {
    status "ERROR" $"Cleanup incomplete: ($failures) path(s) could not be removed."
    exit 1
  }

  status "OK" "Cleanup completed."
}
