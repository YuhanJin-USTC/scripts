...
usage() {
  printf '%s\n' \
    "Usage: $0" \
    "" \
    "Restore an Arch Linux WSL setup from the bundled backup payloads." \
...
elif [ -d "$SCRIPT_DIR/backup/archlinux/data" ]; then
  BACKUP_ROOT="$SCRIPT_DIR/backup/archlinux"
else
...
if [ -f "$BACKUP_ROOT/data/sys_config.tar.gz" ]; then
  ...
  status OK "System configurations restored."
else
...
if grep -q "^#en_US.UTF-8 UTF-8" /etc/locale.gen; then
  ...
  status OK "Locale set to en_US.UTF-8."
else
...
if [ ! -d "/etc/pacman.d/gnupg" ]; then
  ...
  status OK "Pacman keyring initialized."
else
...
if [ -f "$BACKUP_ROOT/data/home_config.tar.gz" ]; then
  ...
  status OK "Home configurations restored."
else
...
if [ -f "$BACKUP_ROOT/data/secure_data.tar.gz.gpg" ]; then
  ...
  if echo "$GPG_PASS" | gpg --yes --batch --pinentry-mode loopback --passphrase-fd 0 --decrypt --output "$SECURE_TAR_TMP" "$BACKUP_ROOT/data/secure_data.tar.gz.gpg"; then
    ...
    status OK "SSH/GPG and global Git configuration restored; SSH config left for dot_files/Stow."
  else
    status ERROR "Credential decryption failed."
    exit 1
  fi
else
...
if [ -f "$PKG_LIST_DIR/pkglist-pacman.txt" ]; then
  ...
  status ERROR "pkglist-pacman.txt not found."
  exit 1
fi
...
if ! command -v yay &>/dev/null; then
  ...
  trap 'CODEX_TEMP_CLEANUP=1 rm -rf "\$YAY_TMP_DIR"' EXIT
...
if [ -f "$PKG_LIST_DIR/pkglist-aur.txt" ]; then
  ...
  if [ -n "$AUR_PKGS" ]; then
    ...
    status SKIP "AUR package list is empty."
  fi
else
  status ERROR "pkglist-aur.txt not found."
  exit 1
fi
...
if [ -f "$SHELL_FILE" ]; then
  TARGET_SHELL=$(tr -d '[:space:]' <"$SHELL_FILE")
  ...
  if [ -n "$TARGET_SHELL" ] && [ -x "$TARGET_SHELL" ]; then
    ...
    status OK "Default shell changed to $TARGET_SHELL."
  else
    status WARN "Configured target shell is missing or invalid."
  fi
else
  status SKIP "default_shell.txt not found."
fi

status OK "Restore completed."
