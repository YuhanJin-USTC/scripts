...
function Show-Usage {
  Write-Output @"
Usage: update_iwan_routes.sh [--dry-run|--run]
...
function Invoke-Main {
  ...
  Write-Field -Name "Target" -Value $configPath
  Write-Field -Name "Mode" -Value $mode
  Write-Field -Name "Rule" -Value "managed IPv4 /32; preserve other routes"
  Write-Field -Name "iWAN mode" -Value $config.Mode
  if ($mode -eq "dry-run") {
  ...
  Write-Status -Label "OK" -Message "iWAN route update completed."
