...
usage() {
  printf '%s\n' \
    "Usage: $0 [--dry-run|--run]" \
    "" \
    "Preview or update Arch Linux packages in WSL." \
    "" \
    "Options:" \
...
run_updates() {
  ...
  echo ""
  echo "[2/3] Updating official packages."
  sudo pacman -Syu

  echo ""
  echo "[3/3] Updating AUR packages."
  yay -Sua
}
...
if [ "$MODE" = "dry-run" ]; then
  ...
  run_updates
  echo ""
  status OK "Arch update completed."
fi
