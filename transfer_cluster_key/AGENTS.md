...
These rules apply only under `transfer_cluster_key/` and supplement the
repository-root instructions. Changes confined here use
`root:scripts:transfer_cluster_key` as their V0 event owner.
...
- Do not execute the script for validation. Do not list matching key files,
  ...
  in logs or responses without explicit authorization.

## Validation

- Run `nu --ide-check 100 tsf_clst_key.nu` when Nushell is available.
- Review matching, newest-file selection, destination construction, overwrite
  behavior, and permissions statically. There is no safe runtime dry run.
